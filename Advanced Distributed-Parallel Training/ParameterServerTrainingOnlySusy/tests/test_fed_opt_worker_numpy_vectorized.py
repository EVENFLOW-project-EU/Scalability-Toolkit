...
class TestFedOptWorker:
    def test_compute_max_min_on_sphere_np_implementations_vectorized(
        self,
        center: np.ndarray,
        radius: int,
        ...
        test_method: callable = GMUtils.compute_min_max_on_sphere_vectorized
        ...
        assert expected_min_max[0] == pytest.approx(result[0], abs=0.5)
        assert expected_min_max[1] == pytest.approx(result[1], abs=0.5)
